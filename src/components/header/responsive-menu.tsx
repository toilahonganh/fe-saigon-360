...
export default function ResponsiveMenu({
  ...
  return (
    <div className="fixed top-0 text-white left-0 w-full h-full bg-bannerGray z-[1000] flex flex-col">
      {/* Header Section */}
              ...
              <div className="flex flex-col mt-10 space-y-10 px-2">
                {menu.subFunctions.map((sub) => (
                  <a
                    key={sub.url}
                    href={sub.url}
                    className="text-sm text-gray-800 font-light hover:font-bold transition-all">
                    {sub.name}
                  </a>
                ))}
              </div>
