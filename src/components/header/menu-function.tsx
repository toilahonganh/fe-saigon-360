      ...
      {menuFunctions.map((menuFunction) => (
        <div
          ...
          <div
            ...
            <span className="text-white text-md font-bold hover:text-[#00a5e0] uppercase">{menuFunction.name}</span>
          </div>
          ...
          {activeDropdown === menuFunction.id && (
            <div
              ...
              onMouseEnter={() => handleMouseEnter(menuFunction.id)}
              onMouseLeave={handleMouseLeave}
              className="fixed left-0 top-[110px] text-white w-full bg-gradient-to-r from-transparent via-black/45 to-transparent shadow-xl rounded-lg z-50 transition-all duration-300 ease-in-out">
              <MenuDropDown menuFunc={menuFunction} />
            </div>
          )}
        </div>
      ))}
